...
# ==================== EL ENTORNO DE TEST VIVE EN UNA COPIA, NO EN EL PRINCIPAL ====================
# La señal, el mensaje y el porqué viven en `bin/lib/ancla.sh`, porque esta misma decisión la tienen
# que aplicar TRES scripts —éste, `init-project.sh` y `up.sh`— y tres copias del bloque acabarían
# divergiendo. Ver esa cabecera: por qué la señal es la de git y no el tipo de `.git` ni el id del
# registro, y por qué el escape es un flag y no una variable.
# shellcheck disable=SC1091
. "$SCRIPT_DIR/lib/ancla.sh"
# `ancla_frase` lee el registro, así que hace falta su lector. Es barato y sin efectos: sólo define
# rutas y funciones.
# shellcheck disable=SC1091
...
if ancla_es_principal "$FS_PROJECT_ROOT" && [ "$PERMITIR_PRINCIPAL" = 0 ]; then
    ancla_niega "$FS_PROJECT_ROOT" "$(registro_id_de "$FS_PROJECT_ROOT" 2>/dev/null || true)" \
        "provisionar el entorno de test" "$0 --en-el-principal"
    exit 1
fi
[ "$PERMITIR_PRINCIPAL" = 1 ] && ancla_avisa_escape "$FS_PROJECT_ROOT" "provisionando el entorno de test"
